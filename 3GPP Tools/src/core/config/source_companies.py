...
def get_matching_contributors(original_source: str) -> list:
    if not original_source:
        return []

    sources_clean = SOURCE_REPLACE_REGEX.sub('', str(original_source)).strip().lower()
    cosigners = [item.strip() for item in sources_clean.split(',')]

    found_cosigners = [key for key, regex in SIGNATURE_SYNONYMS_REGEX.items() if regex.search(sources_clean)]
...
def get_matching_contributors(original_sources, others_cosigners, known_cosigners):
    ...
    cosigners = [item.strip() for item in sources_clean.split(',')]
    found_cosigners = [key for key, regex in SIGNATURE_SYNONYMS_REGEX.items() if
                       regex.search(sources_clean) is not None]
