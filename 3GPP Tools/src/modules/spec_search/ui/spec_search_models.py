...
Preserves direct SQLite clause primary keys (clause_pk) per cell for accurate inspection.
"""
...
class SpecEvolutionMatrixModel(QAbstractTableModel):
    ...
    Supports cell-level clause_pk lookup.
    """
    ...
    def __init__(
        ...
        self._cell_pk_map: Dict[Tuple[str, str], int] = {}
        self._row_first_pk: Dict[int, int] = {}
        self._setup_matrix()
    ...
    def _setup_matrix(self):
        ...
        # Map (clause_number, ver_label) -> clause_pk
        for _, r in df.iterrows():
            key = (str(r["clause_number"]), str(r["ver_label"]))
            self._cell_pk_map[key] = int(r["clause_pk"])
        ...
        # Sort version columns chronologically
        ...
        # Populate row-level fallback primary key
        for r_idx, row in self._pivot_df.iterrows():
            c_num = str(row["clause_number"])
            for v_col in self._version_cols:
                pk = self._cell_pk_map.get((c_num, v_col))
                if pk is not None:
                    self._row_first_pk[r_idx] = pk
                    break

    def get_clause_pk(self, row: int, ver_col: str) -> Optional[int]:
        """Retrieves the SQLite primary key for a specific matrix cell."""
        if 0 <= row < len(self._pivot_df):
            clause_num = str(self._pivot_df.iloc[row]["clause_number"])
            return self._cell_pk_map.get((clause_num, ver_col))
        return None

    def get_row_first_pk(self, row: int) -> Optional[int]:
        """Retrieves the first valid primary key for this row."""
        return self._row_first_pk.get(row)
        ...
        if col == 0:
            ...
            if role == Qt.ToolTipRole:
                ...
                if is_match:
                    ...
                    status_prefix = "⚠️ FIRST INTRODUCED IN THIS VERSION\n" if is_first_added else ""
            ...
            if role == Qt.BackgroundRole:
                if is_match:
                    if is_post_cutoff:
                        return QBrush(QColor("#FEF08A"))
                    if is_first_added:
                        return QBrush(QColor("#DCFCE7"))
                    return QBrush(QColor("#F0FDF4"))
                else:
