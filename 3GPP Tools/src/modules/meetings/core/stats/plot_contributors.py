...
def generate_top_contributors_plot(df, export_dir, theme_color, top_count):
    ...
    fig_comp = px.bar(comp_counts.head(top_count).sort_values('Count', ascending=True),
                      x='Count', y='Company', orientation='h',
                      title=f"Top {top_count} Contributing Companies", color_discrete_sequence=[theme_color])
