...
class ExcelExporterThread(QThread):
    ...
    def __init__(self, output_path: Path, rows_data: list, selected_columns: list, mtg_info: dict, auto_open: bool = True):
        super().__init__()
    ...
    def run(self):
        try:
            self.progress.emit("Initializing workbook...")
            ...
            header_border = Border(
                left=Side(style='thin', color='004275'),
                right=Side(style='thin', color='004275'),
                top=Side(style='thin', color='004275'),
                bottom=Side(style='medium', color='003057')
            )

            center_align = Alignment(horizontal="center", vertical="center", wrap_text=True)
            left_align = Alignment(horizontal="left", vertical="center", wrap_text=True)

            center_cols = {"TDoc", "Type", "For", "Agenda Item", "TDoc Status", "My Status"}
            ...
            self.progress.emit("Formatting layout & column widths...")
            for col_idx, col_name in enumerate(self.selected_columns, start=1):
                col_letter = get_column_letter(col_idx)
                max_len = len(str(col_name))

                # Sample up to 100 rows to calculate optimal width
                for row_idx in range(2, min(total_rows + 2, 102)):
                ...
                if col_name == "Title":
                    width = min(max(max_len + 4, 30), 65)
                elif col_name in ["Secretary Remarks", "My Notes", "Abstract"]:
                    width = min(max(max_len + 4, 25), 55)
            ...
            # --- Top Row Freeze & AutoFilter ---
            if total_rows > 0:
                ws.auto_filter.ref = ws.dimensions
            ws.freeze_panes = "A2"

            # --- Save File ---
            self.progress.emit("Saving spreadsheet...")
            ...
            self.finished.emit(False, f"Cannot write to '{self.output_path.name}'. Please close the file if it is open in Excel.")
