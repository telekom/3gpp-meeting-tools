...
* **📊 Cross-Meeting Company Contribution Audit & Reporting:**
  * **Multi-Meeting Filtering Engine:** Filter and audit contributions across arbitrary date ranges, multiple Working Groups (e.g., SA2, SA1, RAN2), specific 3GPP Work Items, and canonical company entities[cite: 35, 45].
  * **Canonical Company Sanitization & Co-Author Rules:** Integrates directly with `CompanySanitizer` to normalize diverse corporate subsidiaries and brand spellings[cite: 6, 45]. Easily toggle between **"Any co-author (joint contribution)"** and **"Primary source only (first listed)"**[cite: 45].
  * **Work Item (WI) Auto-Completing Token Box:** Search and add multiple Work Item acronyms/codes using a tag/chip input backed by `WorkItemsDatabase` autocompletion[cite: 5, 45].
  * **High-Performance Bounded Concurrency & O(1) Cache:** Operates a laptop-safe `ThreadPoolExecutor` capped at 3 worker threads to prevent CPU throttling, memory bloat, and 3GPP server rate-limiting[cite: 2, 45]. Bypasses expensive full-table regex scanning by resolving `Related WIs` directly from 3GPP document sheets and agenda descriptions[cite: 32, 45]. Employs `TDocsParser` sidecar JSON caches (`.xlsx.json`) for sub-second cache hits on previously opened meetings[cite: 3, 45], with a one-click toggle to **"Bypass local cache (re-fetch from 3GPP)"**[cite: 45].
  * **Modeless, Multi-Window Experience:** Operates as an independent, non-blocking top-level window (`Qt.Window`), enabling full simultaneous access to the main meetings tab, individual meeting tables, and email inspectors[cite: 35, 45].
  * **Clean Application Aesthetic:** Renders in neutral monochrome text without distracting colored cells or heavy badge backgrounds, matching the application's clean design theme[cite: 45].
  * **Dedicated Working Group (WG) & Abstract Columns:** Dedicated **WG** column at the start of the table for instant committee attribution, and an **Abstract** column with multi-line tooltip wrapping and popup reading support[cite: 45].
  * **Direct Row Navigation & Context Menu:**
    * **Smart Double-Click:** Double-clicking `WG` or `Meeting` launches that meeting's full table in `TDocsWindow`[cite: 35, 45]; double-clicking `Abstract` opens the clean pop-up reader[cite: 19, 45]; double-clicking `TDoc` (or any other cell) downloads (if missing), extracts, and opens the actual document in Word via `TDocActionThread`[cite: 40, 45].
    * **Right-Click Command Menu:** Context actions to **🗓️ Open Meeting Table**[cite: 45], **📂 Open Docs Folder** (opens the meeting's public FTP `Docs/` directory in your browser)[cite: 45], **📄 Open TDoc Document** (downloads, extracts, and launches the Word document directly)[cite: 40, 45], **🌐 View on 3GU Portal**[cite: 45], **🔗 Copy URL** (copies direct ZIP download link to clipboard)[cite: 45], **📋 Copy TDoc / Title**[cite: 45], and **📝 View Abstract...**[cite: 19, 45].
  * **In-App KPI Summary Bar:** Real-time summary metric cards displaying **Total Contributions** (with WG coverage count), **Agreed / Approved** (total agreed/approved count and percentage of total submissions)[cite: 45], **Collaboration** (% joint vs. solo)[cite: 45], **Top Co-signer** (most frequent partner organization with joint count)[cite: 45], and **Top Work Item** (accounting for multi-entry WIs while filtering out placeholders like `DUMMY` and `Unspecified`)[cite: 45].
  * **Interactive HTML Analytics Dashboard (`📊 Statistics Dashboard...`):** Generates an offline Plotly HTML report with native integer formatting featuring[cite: 44, 45]:
    * **Monthly Contribution Trend:** Stacked bar chart of outcomes grouped chronologically by Meeting End Date / Month (`YYYY-MM`)[cite: 44].
    * **Overall Contribution Outcomes:** Donut chart with accurate counts and percentage tooltips using semantic 3GPP colors (Agreed/Approved in green, Not Treated in grey, Revised in indigo, Noted in amber, etc.)[cite: 44].
    * **Top Co-signers:** Horizontal bar chart displaying partner organizations segmented by Working Group in stacked colors[cite: 44].
    * **Working Group Activity:** Stacked bar chart showing total contributions per Working Group categorized by outcome status[cite: 44].
    * **Work Item & Study Item Allocation:** Ranking of top 20 active Work Items, splitting multi-entry rows and filtering placeholders[cite: 44].
  * **Corporate Deutsche Telekom Magenta Excel Export:** Exports the aggregated, filtered view into a styled `.xlsx` workbook featuring zebra striping, centered alignments, and active download hyperlinks pointing directly to each TDoc's specific meeting Docs folder[cite: 4, 45].
...
3. **The Threading Bridge:** Worker tasks inherit from `QThread` (e.g., `GeneralEmailSyncThread`, `WordAgendaImporterThread`, `ContributionSearchWorker`, `TDocsDownloaderThread`, `LLMExporterThread`)[cite: 43, 45]. The UI dispatches tasks to the thread, and worker threads emit `pyqtSignals` back to the UI to update progress indicators, models, and logs asynchronously[cite: 43, 45].
...
```bash
git clone https://github.com/telekom/3gpp-meeting-tools.git
...
2. The audit dialog opens as a modeless, non-blocking window, allowing you to use other app tabs and meeting windows concurrently[cite: 35, 45].
3. **Configuring Search Filters:**
   * **Working Groups:** Select one or multiple Working Groups via the checkable dropdown (e.g., `SA2`, `SA1`, `RAN2`)[cite: 45].
   * **Date Range:** Define the meeting start and end date boundaries[cite: 45].
   * **Work Items:** Type a WI acronym or code (e.g., `FS_6G_ARC`, `AmbientIoT_Ph2-ARC`) into the token box and press Enter or click **Add**[cite: 45]. Autocompletion guides your input using the `WorkItemsDatabase`[cite: 5, 45]. Multiple tokens can be added as removable chips[cite: 45].
   * **Target Companies:** Filter and check companies from the list populated from `CompanySanitizer`[cite: 6, 45].
   * **Author Match Rule:** Select **Any co-author** to capture joint contributions, or **Primary source only** to match documents where the company is the first listed submitter[cite: 45].
   * **Bypass Cache:** Check **Bypass local cache (re-fetch from 3GPP)** to force downloading fresh TDoc lists from the 3GPP portal[cite: 45].
4. Click **🔍 Search Contributions**[cite: 45]. The background engine searches matching meetings concurrently (max 3 worker threads), leveraging O(1) sidecar JSON caches to display results without freezing your PC[cite: 2, 3, 45].
5. **Reviewing Results & In-App KPIs:**
   * Review the real-time KPI strip: **Total Contributions** (with WG coverage count), **Agreed / Approved** (total count and percentage of total submissions)[cite: 45], **Collaboration** (% joint vs. solo)[cite: 45], **Top Co-signer**[cite: 45], and **Top Work Item**[cite: 45].
   * Inspect the neutral, uncolored results table containing `WG`, `Meeting`, `TDoc`, `Title`, `Source`, `Matched Company`, `Type`, `For`, `Agenda Item`, `TDoc Status`, `Related WIs`, and `Abstract`[cite: 45].
6. **Row Actions & Direct Navigation:**
   * **Open Meeting Table:** Double-click the **WG** or **Meeting** cell (or right-click and select **🗓️ Open Meeting Table**) to launch that meeting's full table in `TDocsWindow`[cite: 35, 45].
   * **Open TDoc Document:** Double-click the **TDoc** cell (or right-click and select **📄 Open TDoc Document**) to automatically download, extract, and open the document directly in Microsoft Word via `TDocActionThread`[cite: 40, 45].
   * **Open Docs Folder:** Right-click and choose **📂 Open Docs Folder** to open the meeting's public FTP documents directory in your browser[cite: 45].
   * **View on 3GU Portal:** Right-click and select **🌐 View on 3GU Portal** to open the official portal page for that TDoc[cite: 45].
   * **Copy URL:** Right-click and choose **🔗 Copy URL** to copy the direct ZIP download link to your clipboard[cite: 45].
   * **Read Abstract:** Double-click the **Abstract** cell (or right-click and select **📝 View Abstract...**) to open the popup text reader[cite: 19, 45].
7. **Exporting Data & Analytics:**
   * Click **📥 Export to Excel...** to generate a styled Deutsche Telekom magenta `.xlsx` spreadsheet with active download hyperlinks pointing directly to each TDoc's specific meeting Docs folder[cite: 4, 45].
   * Click **📊 Statistics Dashboard...** to compile and open an interactive offline HTML Plotly report displaying monthly contribution trends, overall decision outcomes, top co-signers segmented by Working Group, Working Group activity breakdowns, and Work Item allocations[cite: 44, 45].
