...
1. **The UI Layer (`src/modules/*/ui/`):** Contains Qt Widgets and `QAbstractTableModel` implementations. It never blocks the main GUI thread.
2. **The Core Layer (`src/modules/*/core/`):** Contains domain logic. All database transactions (`sqlite3` with FTS5 trigrams), FTP network scraping (`requests`), COM automation (`win32com` & `pythoncom`), headless LibreOffice conversions, and direct XML manipulation (`lxml` & `python-docx`) are isolated here.
3. **The Threading Bridge:** Worker tasks inherit from `QThread` (e.g., `GeneralEmailSyncThread`, `WordAgendaImporterThread`, `ContributionSearchWorker`, `TDocsDownloaderThread`, `LLMExporterThread`). The UI dispatches tasks to the thread, and worker threads emit `pyqtSignals` back to the UI to update progress indicators, models, and logs asynchronously.
4. **The Singleton Managers & Security Utilities:** Network configuration (`NetworkSession`), cryptographic credential protection (`core.utils.dpapi.SecureCredentialStore`), Word configuration (Sensitivity Labels), database maintenance handlers, and Comparison Cart states are managed by thread-safe singletons and dynamic JSON config loaders to ensure cross-tab synchronization and zero-plaintext credential persistence.
...
6. *(Optional)* **Microsoft Visio** (To view and edit generated `.vsdx` files)
7. *(Optional)* **Microsoft PowerPoint** (For `.pptx` to `.vsdx` conversions)
...
5. **Managing Read & Ignored Statuses:**
   ...
   * **`🚫 Ignore`:** Suppresses high-volume mailing list announcements or bulk compilation emails from badge counts across all referenced TDocs without deleting them from the database.
