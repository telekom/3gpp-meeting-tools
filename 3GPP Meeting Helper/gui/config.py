...
class NetworkConfigDialog:
    ...
    def __init__(self, parent, favicon):
        ...
        self.proxy_server.insert(0, server.default_http_proxy)
        ...
        tkinter.Checkbutton(top, text="Digest Auth.", variable=self.auth_digest_chosen).grid(row=3, column=0, sticky="EW")
        ...
        self.meeting_server.insert(0, server.private_server)
        self.meeting_server.grid(row=4, column=1, columnspan=2,  sticky="EW")
        tkinter.Button(top, text="Apply FTP meeting server address", command=self.store_meeting_ftp_address).grid(row=5, column=1, sticky="EW")
    ...
    def ok(self):
        ...
        if self.auth_digest_chosen.get() == 0:
            ...
            try:
                ...
                server.non_cached_http_session.proxies = proxies
            except:
            ...
            try:
                ...
                if len(user)>0:
                    the_password = self.proxy_password.get()
                    server.non_cached_http_session.auth = HTTPProxyDigestAuth(user, the_password)
    ...
    def ko(self):
        ...
        server.non_cached_http_session.proxies = None
        self.top.destroy()

    def store_meeting_ftp_address(self):
        server.update_meeting_ftp_server(self.meeting_server.get())
        self.meeting_server.delete(0, 'end')
        self.meeting_server.insert(0, server.private_server)
        gui.main.update_ftp_button()
