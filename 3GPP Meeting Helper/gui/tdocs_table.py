...
def set_column(tree, col, label=None, width=None, sort=True, center=True):
    if label is None:
        label = col
    if sort:
        tree.heading(col, text=label, command=lambda: treeview_sort_column(col, False))
    else:
...
class TdocsTable:
    ...
    def __init__(self, parent, favicon):
        ...
        self.search_text.trace_add('write', self.select_text)
    ...
    def insert_rows(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            if self.revisions is None:
                ...
                number_format = '{0:02d}'
                try:
                    revision_count = number_format.format(self.revisions.loc[idx, 'Revisions'])
                except KeyError:
                    # Not found
                    revision_count = number_format.format(0)
                    pass
                except:
                    revision_count = number_format.format(0)
                    traceback.print_exc()
...
class RevisionsTable:
    ...
    def __init__(self, parent, favicon, tdoc_id, revisions_df):
        top = self.top = tkinter.Toplevel(parent)
    ...
    def on_double_click(self, event):
        ...
        if column == 1:
            if actual_value == 0:
                ...
                tdoc_value = tdoc_id + 'r' + '{0:02d}'.format(actual_value)
            print('Opening {0}'.format(tdoc_value))
            gui.main.download_and_open_tdoc(tdoc_value)
        if column == 2:
            pass
        if column == 3:
            pass
